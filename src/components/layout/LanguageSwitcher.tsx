...
// Native/original language names - always shown in their authentic spelling
...
// Optional: Add geographical context for better UX
...
export function LanguageSwitcher() {
  ...
  const [currentLang, setCurrentLang] = useState<SupportedLanguage>(
    () => (i18n.language?.slice(0, 2) as SupportedLanguage) || "sv",
  );
  ...
  const currentLanguageName = NATIVE_LANGUAGE_NAMES[currentLang] || "English";
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={`Current language: ${NATIVE_LANGUAGE_NAMES[currentLang]}. Click to change language.`}
        >
        ...
        {SUPPORTED_LANGUAGES.map((lang) => (
          ...
          >
            <div className="flex items-center gap-2">
              <span className="text-sm">{NATIVE_LANGUAGE_NAMES[lang]}</span>
              <span className="text-xs text-muted-foreground">
                ({lang.toUpperCase()})
              </span>
            </div>
